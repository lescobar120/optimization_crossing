...
async def hello(url, request_data):
    ...
    try:
        async with websockets.client.connect(url, ping_interval=20) as websocket:
            ...
            # IMMEDIATELY RETURN the response - don't wait for more messages
            print("\nReceived response from Bloomberg. Closing connection...")
            return final_response
...
#     Returns:
#         str: XML response from Bloomberg (or None if no response)
#     """
#     try:
#         async with websockets.client.connect(url, ping_interval=20) as websocket:
...
#             else:
#                 print("\nNo response received. Closing connection...")
...
# Main workflow: sends order and returns response
...
#     print("Sent order request... waiting for Bloomberg response.\n")
...
async def interactive_whatif(websocket, request_data):
    """
    Send order and attempt to receive response.
    
    Returns:
        str: XML response from Bloomberg if available, None otherwise
    """
    # Send the order
    await websocket.send(request_data)
    print("Sent order request to Bloomberg.\n")
    
    # Try to receive response with timeout
    try:
        print("Waiting up to 5 seconds for response...")
        final_response = await asyncio.wait_for(websocket.recv(), timeout=15.0)
        print("Received response from Bloomberg!")
        ...
        print("\nNo response received after 5 seconds.")
        print("Orders may have staged successfully without confirmation message.")
        print("Check Bloomberg AIM to verify order status.")
        return None
...
#     except asyncio.TimeoutError:
#         print("ERROR: Timed out waiting for Bloomberg response (30 seconds)")
#         print("Bloomberg may have rejected the request or connection is too slow.")
#         return None
#     except websockets.exceptions.ConnectionClosed as e:
#         print(f"ERROR: Connection closed by Bloomberg: {e}")
#         return None
